  ...
  checkResume() {
    this.service.check_resume({ user_id: sessionStorage.getItem("user_id") }).subscribe((res) => {
      ...
      if (result[0].resume_doc_path) {
        this.status = true;
        this.url = "http://172.26.6.40:8000/vcsapi/get/resume/" + sessionStorage.getItem("user_id") + "/" + sessionStorage.getItem('user_name') + "_resume";
      }
      else {
        this.status = false;
        this.url = "";
    ...
    this.service.uploadFileRegisterGuest(formData, param1, param2, this.applicantForm.value.email).subscribe(res => {
      ...
      if (result.body !== undefined) {
        if (result.body.message === "success") {
          ...
          this.status = true;
          this.url = "http://172.26.6.40:8000/vcsapi/get/resume/" + result.body.user_details.user_id + "/" + sessionStorage.getItem('user_name') + "_resume";
        }
      }
      else {
        this.status = false;
