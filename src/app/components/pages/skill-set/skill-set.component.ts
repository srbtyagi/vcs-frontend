...
export class SkillSetComponent implements OnInit {
  ...
  constructor(public router: Router, public service: RecruiteeService, public route: ActivatedRoute,public fb: FormBuilder) { }
  ...
  getUser(){
    ...
    this.service.getCandidateByUserId({user_id:this.user}).subscribe((res) => {
      //console.log(res,"RESULT");
      let result: any = res;
      if(result.length>0){

        let edit_date=moment(new Date()).format("MM/DD/YYYY");
        if(result[0].edit_date!==undefined || result[0].edit_date!==null || result[0].edit_date!==""){
  ...
  getJObSkillDomain(){
    this.route.queryParams.subscribe((r: any) => {
      // //console.log(JSON.parse(r.special));
      
      //console.log("user",this.user)
      this.service.geAllJobDomain_skillset({skill_area_id:JSON.parse(r.special),user_id:this.user}).subscribe((res) => {
        //console.log(res,"RESULT");
        let result: any = res;
        if(result.length>0){
  ...
  skillRateChange(k,i,data)
  {
    ...
    this.jobDomain[k]["skillset"][i]["date_of_completion"]=moment(new Date(data)).format("MM/DD/YYYY");
    }
  ...
  addCandidate(){
    ...
    //console.log(obj);
    this.service.insertCandidate(obj).subscribe((res) => {
      // //console.log(res,"RESULT");
      let result: any = res;
      if(result==="success"){
      ...
      // {
      //   this.service.spinnerHide();
      //   this.error("Skill Set already Added for other Domain.");
      // }
      else{
        this.service.spinnerHide();
        this.error('Something went wrong,please try again.');
