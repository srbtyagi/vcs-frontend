...
export class RegisterComponent implements OnInit {
  ...
  register() {
    ...
    //console.log(obj)
    this.service
      .register(obj, this.registerexist, this.fileExistData)
      .subscribe(
        (res) => {
          //console.log(res);
          let result: any = res;
          if (result.message === 'You are login') {
            ...
            sessionStorage.setItem(
              'user_name',
              result.user_details.user_first_name
            );
  ...
  checkUniqueEmail() {
    ...
    this.service
      .check_email_register({ email: this.registerForm.value.email })
      .subscribe((res) => {
        //console.log(res);
        let result: any = res;
        if (result.user_id) {
    ...
    this.service
      ...
      .subscribe((res) => {
        ...
        if (result[0].resume_doc_path) {
          this.status = true;
          this.url =
            'https://p3vw3hoyqd.execute-api.us-east-2.amazonaws.com/default/employeeApi-staging/vcsapi/get/resume/' +
  ...
  uploadFiles(file, param1, param2) {
    ...
    this.service.uploadFileRegister(formData, param1, param2).subscribe(
      (res) => {
        let result: any = res;
