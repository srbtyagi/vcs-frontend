...
(function ($) {
  ...
  $.fn.tagComplete = function (userOpts) {
    ...
    return this.each(function (instanceNo, userInputDom) {
      ...
      var instanceId =
        instanceNo + new Date().getTime() + Math.floor(Math.random());

      //main container
      tagCompleteMain = $(
        "<div id='" +
          instanceId +
          "' class='tag_complete_main'>" +
      ...
      //container
      tagInput
        .on("focus", instanceData, function (e) {
      ...
      tagInput.on("keyup", instanceData, function (e) {
        ...
        if (options.freeEdit && value.length == 0 && keycode == backspaceKey) {
          ...
          if ((options.freeEdit && options.freeInput) == true) {
        ...
        //create tag
          ...
          (keycode == enterKey || e.key == options.tokenizer) &&
          value.length > 0 &&
        ...
        if (dataUrl != null) {
          ...
          ajaxReq = $.ajax(ajaxOpts).done(function (data) {
            //proccess data
  ...
  $.fn.addTag = function (tagId, tagText, instance) {
    ...
    if (
      instance.options.onAdd.call(null, {
        tagId: tagId,
        tagText: tagText,
      })
    ) {
  ...
  //update AutoComplete
  $.fn.updateAutoComplete = function (autoCompleteDom, data) {
    var listData = "";
   ...
   * refreshUserInput
   */
  $.fn.refreshUserInput = function (userInput, tagsContainer, options) {
