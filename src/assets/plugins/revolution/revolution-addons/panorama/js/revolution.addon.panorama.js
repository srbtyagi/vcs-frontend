...
(function () {
  ...
  Panorama.prototype = {
    ...
    onReady: function (e, data) {
      ...
      this.panos.each(function (i) {
        $this.slides[i] = new PanoramaSlide(
          $(this),
          $this.slider,
          $this.width,
          $this.height
    ...
    layerAction: function (e, data) {
      ...
      if (action && action.search("panorama") !== -1 && this.currentSlide) {
        var perc = data.hasOwnProperty("percentage")
  ...
  function PanoramaSlide(slide, slider, width, height) {
    ...
    this.camera = new THREE.PerspectiveCamera(
      ...
      width / height,
      1,
      parseInt(options.cameraFar, 10)
    );

    this.camera.target = new THREE.Vector3(0, 0, 0);

    var sphere = new THREE.SphereGeometry(
      parseInt(options.sphereRadius, 10),
      parseInt(options.sphereWsegments, 10),
      parseInt(options.sphereHsegments, 10)
    );
    ...
    this.canvas = $(this.renderer.domElement)
      ...
      .appendTo(slide.find(".slotholder"));
  ...
  PanoramaSlide.prototype = {
    ...
    render: function () {
      ...
      if (this.distX !== false || this.distY !== false) {
        ...
        if (
          ...
          Math.abs(xx - this.x) < 1 &&
          Math.abs(yy - this.y) < 1
        )
          this.controller = false;
      } else if (this.thrw) {
        if (this.mousedown) {
    ...
    zooming: function (e, scale) {
      ...
      if (this.zoomVal === this.zoomMax || this.zoomVal === this.zoomMin)
        clearInterval(this.zoomTimer);
  ...
  function setActions() {
    ...
    while (i--) {
      ...
      if (action.action.search("panorama") !== -1)
        $this
          .addClass("rspanoaction")
          .on("mousedown click mousemove touchmove", stopBubble);
    }
  }
