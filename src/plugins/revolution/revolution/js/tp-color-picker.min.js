...
window.RevColor = {
  ...
  convert: function (b, k) {
    ...
    if (
      ...
      return RevColor.process(b, !0)[0];
    k = parseFloat(k);
    1 >= k && (k *= 100);
    k = Math.max(Math.min(parseInt(k, 10), 100), 0);
    if (0 === k) return "transparent";
    try {
      if (-1 !== b.search("#") || 8 > b.length)
        return (
          RevColor.isColor.test(b) || (b = b.replace(/[^A-Za-z0-9#]/g, "")),
          RevColor.processRgba(RevColor.sanitizeHex(b), k)
        );
      b = RevColor.rgbValues(b, 3);
      return RevColor.rgbaString(b[0], b[1], b[2], 0.01 * k);
    } catch (l) {
  ...
  process: function (b, k) {
    if ("string" !== typeof b)
      return (
        k && (b = RevColor.sanitizeGradient(b)),
        [RevColor.processGradient(b), "gradient", b]
      );
    if ("transparent" === b.trim()) return ["transparent", "transparent"];
    if (-1 !== b.search(/\[\{/))
      try {
        return (
      ...
      return -1 !== b.search("#")
        ? [RevColor.sanitizeHex(b), "hex"]
        : -1 !== b.search("rgba")
        ? [b.replace(/\s/g, "").replace(/false/g, "1"), "rgba"]
        : [b.replace(/\s/g, ""), "rgb"];
  },
  transparentRgba: function (b, k) {
    return k || "rgba" === RevColor.process(b)[1]
      ? "0" === RevColor.rgbValues(b, 4)[3]
      : !1;
  },
  ...
  getRgbToHex: function (b, k, l) {
    return (
      ...
      ("0" + parseInt(k).toString(16)).slice(-2) +
      ("0" + parseInt(l).toString(16)).slice(-2)
    );
  },
  ...
  processRgba: function (b, k) {
    b = b.replace("#", "");
    var l = "undefined" !== typeof k,
      p =
        (l ? "rgba" : "rgb") +
        "(" +
        parseInt(b.substring(0, 2), 16) +
        ", " +
        parseInt(b.substring(2, 4), 16) +
        ", " +
        parseInt(b.substring(4, 6), 16);
    return (p = l
      ? p +
        (", " + (0.01 * parseInt(k, 10)).toFixed(2).replace(/\.?0*$/, "") + ")")
      : p + ")");
  },
  processGradient: function (b) {
    var k = b.type,
      l = "linear" === k ? b.angle + "deg, " : "ellipse at center, ";
    b = b.colors;
    for (var p = b.length, B = "", u, y = 0; y < p; y++)
      0 < y && (B += ", "),
        (u = b[y]),
        (B +=
          "rgba(" +
  ...
  sanitizeGradient: function (b) {
    for (var k = b.colors, l = k.length, p = [], B, u = 0; u < l; u++) {
      ...
      delete y.align;
      B
        ? JSON.stringify(y) !== JSON.stringify(B) && (p[p.length] = y)
        : (p[p.length] = y);
      B = y;
    }
    b.colors = p;
...
(function (b) {
  ...
  function l() {
    this.setAttribute(
      "placeholder",
      T[this.getAttribute("data-placeholder") || ""]
    );
  }
  function p() {
  ...
  function u(a, c, d, e) {
    if (b.isPlainObject(a)) {
      ...
      for (g in a)
        if (a.hasOwnProperty(g)) {
          ...
          var f = isNaN(g)
            ? g.replace(/_/g, " ").replace(/\b\w/g, function (a) {
    ...
    if ("blank" !== a)
      return (
        b.isPlainObject(a) && ((h = a), (a = e || RevColor.processGradient(a))),
        (a =
  ...
  function y() {
    var a = (this.getAttribute("data-color") || "").toLowerCase(),
      c = ha ? !1 : a === Wa.toLowerCase();
    if (a === ia || c)
      return (
        (a = b(this)),
        a
  ...
  function O(a, c, b) {
    if (A) {
      a ||
        ((c = c || t.val()),
        (b = "undefined" !== typeof b ? b : w.val()),
        (c =
          "transparent" === c
            ? "transparent"
            : "100%" === b
            ? RevColor.sanitizeHex(c)
            : RevColor.processRgba(c, b)));
  ...
  function ya(a, c) {
    ...
    if ("gradient" !== e) {
      switch (e) {
        case "hex":
          a = RevColor.sanitizeHex(g);
          w.val("100%");
          U(100);
          break;
        case "rgba":
          var d = RevColor.rgbaToHex(g),
            h = parseInt(100 * d[1], 10);
          a = d[0];
          w.val(h + "%");
          U(h);
          break;
      ...
      b('.rev-cpicker-orientation[data-direction="' + h + '"]').addClass(
        "selected"
      );
      z.val(Aa(h));
      Xa(h);
      for (
        ...
        (f = d[k]),
          (r = f.align),
          h.appendChild(Ya(r, f.position, Za(f), $a(f, r)));
  ...
  function cb(a) {
    ...
    var b = f.attr("data-color"),
      d = f.hasClass("rev-cpicker-point-bottom");
    if (d)
      E.hasClass("active") &&
        (W.attr("disabled", "disabled"),
        M.attr("disabled", "disabled"),
        E.removeClass("active")),
        (b = RevColor.rgbaToHex(b)[0]),
        X.css("background", b),
        N.removeAttr("disabled").val(a + "%"),
        2 < n.find(".rev-cpicker-point-bottom").length && F.addClass("active"),
        n.addClass("open");
    else {
      F.hasClass("active") &&
        (X.css("background", ""),
  ...
  function Da(a, b) {
    ...
    q
      ? (q.hasClass("selected")
          ? (b && m.data("hex", q.attr("data-color").toLowerCase()),
    ...
    b ||
      (Ea && Ea(),
      G && "transparent" !== G
        ? (A[0].style.background = G)
        : A.css("background", ""),
      I.trigger("revcolorpickerupdate", [m, G]));
    m = A = !1;
  }
  function Fa() {
    var a = b(this).children(".rev-cpicker-color").not(".blank").length;
    6 < a
      ? b("#" + this.id + "-btn").addClass("multiplerows")
      : b("#" + this.id + "-btn").removeClass("multiplerows");
    return a;
  }
  function Bb() {
    for (
      var a = b(this),
        c = -1 !== this.id.search("colors") ? 5 : 6,
        a = a.children(".rev-cpicker-color"),
        d = a.length,
        e = Math.ceil(d / 6),
        c = 6 * c,
        g,
  ...
  function x(a, b, d) {
    ...
    eb();
    Ha = !1;
    for (var c, g = [], h = v.length, f = 0; f < h; f++)
      (c = v[f]),
        (b = c.color),
        (g[f] = b),
        (c = c.el),
        c.setAttribute("data-color", RevColor.rgbaString(b.r, b.g, b.b, b.a)),
        c.setAttribute("data-opacity", 100 * b.a),
        a && a === c && (na = f);
    fb.hasClass("selected")
      ? ((C.type = "radial"), (C.angle = "0"))
      : ((C.type = "linear"), (C.angle = parseInt(z.val(), 10).toString()));
    C.colors = g;
  ...
  function Ia(a, b, d, e, g) {
    return Math.max(
      Math.min(Math.round(Math.abs(((a - b) / (d - b)) * (g - e) + e)), 255),
      0
    );
  }
  function eb() {
    ...
    Q.each(Db);
    v.sort(Ba);
    for (var a = v.length, b = a - 1, d, e, g, h, f = 0; f < a; f++)
      if (
        ((d = v[f]),
        (g = d.align),
        (e = hb(f, g)),
        ...
        r !== k
          ? ((e = e.x),
            (h = Math.max(
              Math.min(
                Math.abs(
                  parseFloat((((g.x - e) / (h.x - e)) * (k - r)).toFixed(2)) +
                    parseFloat(r)
                ),
                1
              ),
        ...
        (g = d.color),
          ...
          e !== h
            ? ((d = d.x),
              (e = e.x),
              (h = h.x),
              (g.r = Ia(d, e, h, k.r, r.r)),
              (g.g = Ia(d, e, h, k.g, r.g)),
              (g.b = Ia(d, e, h, k.b, r.b)))
            : ((g.r = k.r), (g.g = k.g), (g.b = k.b));
    v.sort(Ba);
  }
  function Ba(a, b) {
  ...
  function Db(a) {
    var b = RevColor.rgbValues(this.getAttribute("data-color") || "", 4),
      d = -1 !== this.className.search("bottom") ? "bottom" : "top",
      e = b[3].replace(/\.?0*$/, "") || 0,
      g = parseInt(this.style.left, 10);
    Ha &&
      ((g = 50 > g ? g + 2 * (50 - g) : g - 2 * (g - 50)),
      (this.style.left = g + "%"),
      this.setAttribute("data-location", g));
    v[a] = {
      el: this,
      x: g,
      alpha: e,
    ...
    f &&
      f[0] !== this &&
      ("bottom" === d ? (ba[ba.length] = g) : (aa[aa.length] = g));
  }
  function Xa(a) {
    ...
    if (a && "keyup" === a.type) {
      ...
      d !== a &&
        ...
        e ||
          ((d = c || d),
          P.removeClass("selected"),
          b('.rev-cpicker-orientation[data-direction="' + d + '"]').addClass(
            "selected"
          )));
    }
    if (g || c) h && Xa(h), x();
  ...
  function kb() {
    ...
    -1 !== this.className.search("down")
      ? (a.parent().addClass("active"),
        a.closest(".rev-cpicker-presets").addClass("active"),
        b(this.id.replace("-btn", "")).addClass("active"),
        (qa = n.hasClass("gradient-view")))
      : (a.parent().removeClass("active"),
        a.closest(".rev-cpicker-presets").removeClass("active"),
        b(this.id.replace("-btn", "")).removeClass("active"),
        (qa = !1));
  }
  function Eb(a, b) {
    var c = parseInt(100 * (Math.round(b.position.left) / 263).toFixed(2), 10);
    "bottom" === Ka
      ? N.val(c + "%").trigger("keyup")
      : M.val(c + "%").trigger("keyup");
  }
  function Fb() {
    var a = b(this);
  ...
  function lb(a) {
    ...
    isNaN(d) ||
      isNaN(a) ||
      ((d = (180 / Math.PI) * Math.atan2(a - 30, d - 30) + 90),
      0 > d && (d += 360),
      (d = Math.max(0, Math.min(360, Math.round(d)))),
      (d = 5 * Math.round(d / 5)),
      Ja(!1, !1, d));
  }
  function da(a) {
  ...
  function Hb() {
    ...
    D.data(
      "state",
      ob.find(".rev-cpicker-color").eq(0).attr("data-color") || "#ffffff"
    );
    H.data(
      "state",
      nb.find(".rev-cpicker-color").eq(0).attr("data-color") ||
        "linear-gradient(0deg, rgba(255, 255, 255, 1) 0%, rgba(0, 0, 0, 1) 100%)"
    );
    Z.draggable({
      containment: "window",
    ...
    b(".rev-cpicker-main-btn").on("click.revcpicker", function () {
      ...
      xa
        ? (A && (ia = t.val()),
          n.removeClass("gradient-view").addClass("color-view"))
        : (A && (ia = a),
          n.removeClass("color-view").addClass("gradient-view"),
          ab || Pa.children(".rev-cpicker-color").not(".blank").eq(0).click());
    ...
    b("#rev-cpicker-check").on("click.revcipicker", function () {
      if (n.hasClass("color-view")) {
        ...
        a =
          "transparent" === a
            ? "transparent"
            : "100%" === c
            ? RevColor.sanitizeHex(a)
            : RevColor.processRgba(a, c);
        c = [m, a, !1];
      } else {
        db();
        a = x(!1, !1, !0);
      ...
      if ((d = c[1] !== G))
        m.attr("data-color", c[1]).val(a).change(),
          I.trigger("revcolorpicker", c),
          Ra && Ra(c[0], c[1], c[2]);
      Da(!1, d);
    });
    n.on("click.revcpicker", function (a) {
      if (n.hasClass("open")) {
        ...
        var d = c.id;
        (c =
          -1 !== c.className.search("rev-cpicker-point") ||
          "rev-cpicker-section-right" === d ||
          -1 !== d.search("hit") ||
          a.closest("#rev-cpicker-section-right, #rev-cpicker-point-wrap")
            .length) &&
          ("text" === a.attr("type")
            ? (c = !a.attr("disabled"))
            : "rev-cpicker-check-gradient" === d && (c = !1));
        c || db();
      } else Sa && !1 === /wheel|angle|reverse/.test(a.target.id) && (-1 === a.target.id.search("radial") && b('.rev-cpicker-orientation[data-direction="' + parseInt(z.val()) + '"]').addClass("selected"), ca.removeClass("active"), (Sa = !1));
    });
    ...
    R.wpColorPicker({
      ...
      change: function (a, b) {
        ...
        var c = RevColor.processRgba(c, 100),
          e = RevColor.rgbValues(c, 4),
          g = C.colors[na];
        g.r = e[0];
        g.g = e[1];
        g.b = e[2];
    ...
    b(".rev-cpicker-point-location")
      .on("keyup.revcpicker focusout.revcpicker", function (a) {
        if (f) {
          var b = f.hasClass("rev-cpicker-point-bottom") ? "bottom" : "top",
            d = "bottom" === b ? ba : aa,
            b = "bottom" === b ? N : M,
            e = b.val().replace("%", "") || "0";
          a = a.type;
          ...
          "focusout" === a && (b.val(e + "%"), f.attr("data-location", e));
          f.css("left", e + "%");
          x();
        }
      })
    ...
    b("body")
      ...
      .on("mousemove.revcpicker", function (a) {
        if (f && f.data("mousestart")) {
          ...
          a = a.pageY;
          f.hasClass("rev-cpicker-point-bottom")
            ? a > b && 15 < a - b && F.hasClass("active")
              ? f.addClass("warning")
              : f.removeClass("warning")
            : b > a && 15 < b - a && E.hasClass("active")
            ? f.addClass("warning")
            : f.removeClass("warning");
        }
      })
      .on("mouseup.revcpicker", function (a) {
        if (f && f.data("mousestart")) {
          ...
          f.hasClass("rev-cpicker-point-bottom")
            ? a > b && 10 < a - b && F.hasClass("active")
              ? F.click()
              : f.removeClass("warning")
            : b > a && 10 < b - a && E.hasClass("active")
            ? E.click()
            : f.removeClass("warning");
        }
      })
      ...
      .on("click.revcpicker", ".rev-cpicker-color:not(.blank)", function () {
        ...
        q = b(this);
        var a = q.parent()[0].id,
          c = -1 !== a.search("core") ? "core" : "custom",
          a = -1 !== a.search("colors") ? "colors" : "gradients",
          c = b("#rev-cpicker-" + a + "-" + c + "-btn");
        c.hasClass("active") && c.find(".rev-cpicker-arrow-up").click();
        n.hasClass("color-view")
          ? ((c = q.attr("data-color")),
            (Ta = !0),
            K.val(c).change(),
            "transparent" === t.val() && t.val(c.toLowerCase()),
            (Ta = !1),
            (a = w.val()),
            0 === parseInt(a, 10) && (c = "transparent"),
            O(!1, c, a))
          : (ea.removeClass("full"),
            sa.removeClass("full"),
            ya(q.data("gradient"), !0),
            J.removeClass("checked"),
            Pa.find(".rev-cpicker-color.selected").removeClass("selected"));
        q.addClass("selected");
      })
      .on(
      ...
      .on("click.revcpicker", ".rev-cpicker-delete", function () {
        if (fa) {
          if (
            ...
            var e = a.closest(".rev-cpicker-presets-group")[0].id,
              g = -1 !== e.search("colors") ? "colors" : "gradients";
            I.off("revcpicker_onajax_delete.revcpicker").on(
              "revcpicker_onajax_delete.revcpicker",
              function (d, g) {
                g && console.log(g);
                var h = a.closest(".rev-cpicker-presets-group"),
                  f = h.find(".ps-scrollbar-x-rail"),
                  k = b("#" + e + "-btn");
                c.remove();
                Bb.call(h[0])
                ...
                7 > Fa.call(h[0]) &&
                  ...
                  k.hasClass("active") &&
                    k.children(".rev-cpicker-arrow-up").click());
                n.removeClass("onajax onajaxdelete");
              }
            );
            d = b
              .trim(d.replace(/\W+/g, "_"))
              .replace(/^\_|\_$/g, "")
              .toLowerCase();
          ...
          b("#rev-cpicker-" + e + "-custom")
            ...
            .each(function () {
              if (
                b
                  .trim(
                    (this.getAttribute("data-title") || "").replace(/\W+/g, "_")
                  )
                  .replace(/^\_|\_$/g, "")
                  .toLowerCase() === d
              )
                return alert(c.attr("data-message")), (a = !0), !1;
            });
          if (!a) {
            ...
            if ("colors" === e) {
              ...
              h =
                "transparent" === h
                  ? "transparent"
                  : "100%" === f
                  ? RevColor.sanitizeHex(h)
                  : RevColor.processRgba(h, f);
            } else {
              var k = pa.style.background;
              h = b.extend({}, x(!1, !1, !0)[0]);
            }
            g[d] = h;
            I.off("revcpicker_onajax_save.revcpicker").on(
              "revcpicker_onajax_save.revcpicker",
              function (a, d) {
                if (d)
                  n.removeClass("onajax onajaxsave"),
                    alert(c.attr("data-message"));
                else {
                  var h = b(u(g, !1, " rev-picker-color-custom", k)),
                    f = b("#rev-cpicker-" + e + "-custom"),
                    r = f.find(".rev-cpicker-color.blank"),
                    m = b("#" + f[0].id + "-btn");
                  r.length
                    ? h.insertBefore(r.eq(0))
                    : h.insertBefore(f.find(".ps-scrollbar-x-rail"));
                  b("#rev-cpicker-" + e + "-custom-btn").click();
                  6 < Fa.call(f[0]) &&
                    (r.length && r.last().remove(),
                    m
                      .addClass("active")
                      .children(".rev-cpicker-arrow-down")
                      .click(),
                    f.perfectScrollbar("update"));
                  h.click();
                  n.removeClass("onajax onajaxsave");
                }
              }
    ...
    b(".rev-cpicker-preset-title").on("click.revcpicker", function () {
      var a = b(this),
        c = a.parent(),
        d = a.hasClass("active") ? "down" : "up";
      kb.call(a.find(".rev-cpicker-arrow-" + d)[0]);
    ...
    b(".rev-cpicker-input").on("focusin.revcpicker", function () {
      var a = b(this);
    ...
    b(".rev-cpicker-hex")
      .on("focusout.revcpicker", function () {
        ...
        if ("rev-cpicker-color-hex" === this.id) {
          if ((a = t.val()))
            if (((a = RevColor.sanitizeHex(a)), RevColor.isColor.test(a)))
              t.val(a);
            else if (((a = b(this)), (a = a.data("orig-value")))) t.val(a);
            else {
              ka.click();
              return;
            }
          ...
          K.val(a).change();
        } else (a = ra.val() || RevColor.defaultValue), (a = RevColor.sanitizeHex(a)), RevColor.isColor.test(a) || ((a = b(this)), (a = (a = a.data("orig-value")) ? a : RevColor.defaultValue)), ra.val(a), R.val(a).change();
      })
      .on("focusin.revcpicker", da);
    b("#rev-cpciker-clear-gradient").on("click.revcpicker", function () {
      R.val(RevColor.defaultValue).change();
    });
    z.on("keyup.revcpicker focusout.revcpicker", Ja)
      .on("focusin.revcpicker", function () {
        Sa = !0;
        ca.addClass("active");
      })
    ...
    P.on("click.revcpicker", function () {
      var a = b(this),
        c = a.attr("data-direction");
      P.removeClass("selected");
      a.addClass("selected");
      "radial" !== c
        ? z.removeAttr("disabled").val(Aa(c))
        : z.attr("disabled", "disabled");
      Ja(!1, c);
    });
    b(".rev-cpicker-point-delete").on("click.revcpicker", function () {
      if (-1 !== this.className.search("active")) {
        var a = f.hasClass("rev-cpicker-point-bottom") ? "bottom" : "top",
          b = n.find(".rev-cpicker-point-" + a).length;
        2 < b &&
          (f.draggable("destroy").remove(), (Q = L.children()), n.click(), x());
        20 >= b &&
          ("bottom" === a ? sa.removeClass("full") : ea.removeClass("full"));
      }
    });
    b(".rev-cpicker-preset-save").on("focusin.revcpicker", da);
    b(".rev-cpicker-opacity-input")
      .on("keyup.revcpicker focusout.revcpicker", function (a) {
        var c = -1 === this.id.search("grad"),
          d = c ? w : W,
          e = d.val().replace("%", "");
        a = a.type;
        ...
        c
          ? ((c = parseInt(e, 10)),
            O(!1, 0 !== c ? !1 : "transparent", e),
            U(e))
          : ((c = RevColor.rgbValues(f.attr("data-color"), 3)),
            (d = C.colors[na]),
            (e = (0.01 * parseInt(e, 10)).toFixed(2).replace(/\.?0*$/, "")),
            (d.r = c[0]),
            (d.g = c[1]),
            (d.b = c[2]),
            (d.a = e),
            (c = RevColor.rgbaString(d.r, d.g, d.b, e)),
            f.attr("data-color", c),
            x(),
            (c = "rgba(0, 0, 0, " + e + ")"),
            (la.style.backgroundColor = c),
            (ma.style.borderTopColor = c));
      })
      .on("focusin.revcpicker", da);
    b(".rev-cpicker-builder-hit").on("click.revcpicker", function (a) {
      ...
      for (
        var b = -1 !== this.id.search("bottom") ? "bottom" : "top",
          d = "bottom" === b ? ba : aa,
          e = 50 > a ? 1 : -1;
        -1 !== d.indexOf(a);
        ...
        a += e;
      "bottom" === b
        ? 20 > n.find(".rev-cpicker-point-bottom").length
          ? (bb(b, a), (q = !1))
          : sa.addClass("full")
        : 20 > n.find(".rev-cpicker-point-top").length
        ? (bb(b, a), (q = !1))
        : ea.addClass("full");
    });
    ...
    for (var e = 0; e < d; e++) {
      ...
      for (
        var n = document.createDocumentFragment(),
          m = -1 !== f.search("core"),
          q = m ? "" : " rev-picker-color-custom",
          l = k.length,
          p = -1 !== f.search("colors") ? 5 : 6,
          l = Math.max(Math.ceil(l / 6), p),
          p = 0;
        p < l;
        p++
      )
        for (; k.length < 6 * (p + 1); ) k[k.length] = "blank";
      l = k.length;
  ...
  var I,
    ...
    Ra,
    C = {},
    V = { axis: "x", containment: "#rev-cpicker-point-wrap" },
    wa = {
      color: "Color",
    ...
    ua =
      "#FFFFFF #000000 #FF3A2D #007AFF #4CD964 #FFCC00 #C7C7CC #8E8E93 #FFD3E0 #34AADC #E0F8D8 #FF9500".split(
        " "
      ),
    va = [
      {
  ...
  b.tpColorPicker = function (a) {
    ...
    a || (a = {});
    a.core &&
      (a.core.colors && (ua = a.core.colors),
      a.core.gradients && (va = a.core.gradients));
    rb(a);
    Na
      ? (S.perfectScrollbar("update"),
        a.mode && (Ua = a.mode),
        a.language && B(a.language))
      : (B(a.language || wa), (Ua = a.mode || "full"));
    a.init && (tb = a.init);
  ...
  var Ib = {
    refresh: function () {
      ...
      if (a.hasClass("rev-cpicker-component")) {
        var c = a.data("revcp") || {},
          c = a.val() || c.defaultValue || RevColor.defaultValue,
          d = RevColor.process(c),
          c = d[0],
          d = "rgba" === d[1] && RevColor.transparentRgba(c, !0) ? "" : c;
        "transparent" !== c
          ? (a.data("tpcp")[0].style.background = d)
          : a.data("tpcp").css("background", "");
  ...
  b.fn.tpColorPicker = function (a) {
    return a && "string" === typeof a
      ...
      : this.each(function () {
          var c = b(this);
          if (c.hasClass("rev-cpicker-component")) c.tpColorPicker("refresh");
          else {
            var d = b('<span class="rev-colorpicker"></span>').data(
                "revcolorinput",
                c
              ),
              e = b('<span class="rev-colorbox" />'),
              g = b('<span class="rev-colorbtn" />'),
              h = c.attr("data-wrap-classes"),
              f = c.attr("data-wrapper"),
              k = c.attr("data-wrap-id"),
              n = c.attr("data-title"),
              m = c.attr("data-skin"),
              l = c.val();
            ...
            c.attr({ type: "hidden", "data-color": l })
              .data("tpcp", e)
              .addClass("rev-cpicker-component");
            m && d.addClass(m);
            f
              ? ((f = b(f).addClass("rev-cpicker-master-wrap")), d.wrap(f))
              : d.addClass("rev-cpicker-master-wrap");
  ...
  b(function () {
    b("body").on("click.revcpicker", ".rev-colorpicker", function () {
      ...
      var a = m.attr("data-widget-id"),
        c = m.attr("data-appended-html"),
        d = m.attr("data-editing"),
        e = m.attr("data-colors"),
        g = m.attr("data-mode"),
        f = m.data("revcp"),
        k = m.attr("data-lang"),
        l;
      if (e) {
        ...
        if (e.gradients) var q = e.gradients;
      }
      if (f) {
        if ((l = f.colors)) {
          if (l.core) {
      ...
      if (u || t || w || v || q || p) {
        f = {};
        if (u || t || q || p)
          f.core = { colors: p || t || ua, gradients: q || u || va };
      ...
      d
        ...
        : ((d = ""), (Oa.style.visibility = "hidden"));
      mb.innerHTML = d;
      "single" === g || "basic" === g
        ? ((ja = !1),
          H.hide(),
          D.show(),
          "basic" === g && n.addClass("is-basic"))
        : ((ja = !0), H.show(), D.show());
